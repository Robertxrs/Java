...
public class TabelaPagamento extends javax.swing.JFrame {

    /**
     * Creates new form TabelaPagamento
     */
    public TabelaPagamento() {
        initComponents();
        PagamentoDAO pagamentoDAO = new PagamentoDAO();
        List<Pagamentos> listaPagamento = pagamentoDAO.listar(-1, -1, "", "", "", -1);
        preencheTabela(listaPagamento);
    }

    public void preencheTabela(List<Pagamentos> pagamentos) {
        String[] columns = {"Id", "Cliente Id", "Tipo", "Status", "Data Pagamento", "Veículo Id"};
        String[][] dados = new String[pagamentos.size()][columns.length];

        int i = 0;
        for (Pagamentos p : pagamentos) {
            dados[i][0] = String.valueOf(p.getId());
            dados[i][1] = String.valueOf(p.getCliente_id());
            dados[i][2] = p.getTipo();
            dados[i][3] = p.getStatus();
            dados[i][4] = p.getData_pagamento() != null ? p.getData_pagamento().toString() : "";
            dados[i][5] = String.valueOf(p.getVeiculo_id());
            i++;
        }
        DefaultTableModel model = new DefaultTableModel(dados, columns);
        TabelaPagamento.setModel(model);
    }
    ...
    private void initComponents() {
        ...
        jLabelTitle.setText("Encontrar pagamento por id");
        ...
        TabelaPagamento.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id", "Cliente id", "Tipo", "Status", "Data de pagamento", "Veiculo Id"
            }
        ));
        TabelaPagamento.setToolTipText("");
    ...
    private void jButtonConfirmarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonConfirmarActionPerformed
        ...
        PagamentoDAO pagamentoDao = new PagamentoDAO();
        List<Pagamentos> resultados = pagamentoDao.obterId(id);
        DefaultTableModel model = (DefaultTableModel) TabelaPagamento.getModel();
        model.setRowCount(0);
        for (Pagamentos pagamento : resultados) {
            Object[] rowData = {pagamento.getId(), pagamento.getCliente_id(), pagamento.getTipo(), pagamento.getStatus(), pagamento.getData_pagamento(), pagamento.getVeiculo_id()};
    ...
    private void jButtonCadastrar1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonCadastrar1ActionPerformed
        CadastrarPagamento cc = new CadastrarPagamento();
        cc.setVisible(true);
    ...
    private void excluirSelecionado() {
        int rowIndex = TabelaPagamento.getSelectedRow();

        if (rowIndex != -1) {
            int pagamentoId = Integer.parseInt(TabelaPagamento.getValueAt(rowIndex, 0).toString());
            PagamentoDAO pagamentoDao = new PagamentoDAO();
            pagamentoDao.excluir(pagamentoId);
            List<Pagamentos> ListaCompra = pagamentoDao.listar(-1, -1, "", "", "", -1);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TabelaPagamento.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new TabelaPagamento().setVisible(true);
            }
        });
    }
