...
public class PodcastDAO {
    ...
    public List<Podcast> listar(String filtroProdutor, String duracao, Integer numeroEpisodio,
            String filtroEpisodio, String filtroUrl) {
        EntityManager em = JPAUtil.getEntityManager();
        ...
        try {
            String textoQuery = "SELECT p FROM Podcast p "
                    ...
                    + "AND (:numeroEpisodioFiltro = -1 OR p.numeroEpisodio >= :numeroEpisodioFiltro) "
                    + "AND (:episodioFiltro IS NULL OR p.episodio LIKE :episodioFiltro) "
            ...
            consulta.setParameter("episodioFiltro", filtroEpisodio.isEmpty() ? null : "%" + filtroEpisodio + "%");
